...
/*
   Add this HTML to your dashboard above the booking list/table:
   <div style="margin-bottom: 15px;">
     <input type="date" id="searchDate" style="padding: 5px;" />
     <button onclick="searchBookingsByDate()" style="padding: 5px 10px;">Search</button>
     <button onclick="location.reload()" style="padding: 5px 10px;">Reset</button>
   </div>
*/
...
async function searchBookingsByDate() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // Logic to update the Provider Dashboard table
      const tbody = document.getElementById('bookingTableBody'); 
      if (tbody) {
        ...
        UI.toast('Check console for results', 'info');
      }
    } else {
...
/*
  Add this HTML to your service details page to display reviews with a sort dropdown.
  (Remember to replace the `serviceId` in `onchange` with a dynamic value if needed).

  <div style="margin-bottom: 15px; display: flex; align-items: center; gap: 10px;">
    <label for="reviewSort" style="font-weight: bold;">Sort by:</label>
    <select id="reviewSort" onchange="loadServiceReviews(1)" style="padding: 5px; border-radius: 6px; border: 1px solid #ccc;">
...
async function markReviewHelpful(btn, reviewId) {
  try {
    ...
    btn.style.color = '#28a745'; // Change color to indicate success
